...
const fetchPost = async (q) => {
  const res = await axios.get(`${process.env.NEXT_PUBLIC_POST}/search/${q}`);
  const data = await res.data;
  return data;
};

const Page = async ({ searchParams: { q } }) => {
  const posts = await fetchPost(q);
  return (
    <main className="mt-[65px] mx-auto md:max-w-6xl">
      <p className="px-3">Search results for "{q}"</p>
