...
const Page = () => {
  ...
  const [rating, setRating] = useLocalStorage("rating", "");
  const [language, setLanguage] = useLocalStorage("language", "");
  const [category, setCategory] = useLocalStorage("category", "");
  const [rom, setRom] = useLocalStorage("rom", "");
  const [isdraft, setIsDraft] = useLocalStorage("isdraft", "");
  const [file, setFile] = useLocalStorage("romfile", "");
  const [image, setImage] = useLocalStorage("image", "");
  const [permalink, setPermalink] = useLocalStorage("permalink", "");
  ...
  const publishPost = async () => {
    const res = await axios.post(`${process.env.NEXT_PUBLIC_POST}`, {
      title,
      name,
      size,
      rating,
